...
class Tensor:
    ...
    def grad_add(self, gradients=None):
        ...
        """
        # TODO
        grad1=self.history[1]
        grad2=self.history[2]

        grad1.grad=np.zeros_like(grad1.arr)
        grad2.grad=np.zeros_like(grad2.arr)

        if grad1.requires_grad:
            grad1.grad = grad1.grad + np.ones_like(grad1.arr)
        
        if grad2.requires_grad:
            grad2.grad = grad2.grad + np.ones_like(grad2.arr)

        if gradients is None:
            return(grad1.grad,grad2.grad)

        if grad1.requires_grad:
            grad1.grad = np.multiply(np.ones_like(grad1.arr))

        if grad2.requires_grad:
            grad2.grad = np.multiply(np.ones_like(grad1.arr))
    ...
    def grad_matmul(self, gradients=None):
        ...
        if gradients is None:
            if grad1.requires_grad:
                grad1.grad = grad1.grad + np.matmul(np.ones_like(grad1.arr),grad2.arr.transpose())
            if grad2.requires_grad:
                grad2.grad = grad2.grad + (np.matmul(np.ones_like(grad2.arr),grad1.arr)).transpose()

        else:
            if grad1.requires_grad:
                grad1.grad = grad1.grad + np.multiply(np.matmul(np.ones_like(grad1.arr),grad2.arr.transpose()),gradients)
            if grad2.requires_grad:
                grad2.grad = grad2.grad + np.multiply(np.matmul(np.ones_like(grad2.arr),grad1.arr).transpose(),gradients)
